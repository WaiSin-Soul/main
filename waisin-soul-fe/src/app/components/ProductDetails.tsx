      ...
      <div className="text-sm text-gray-400 mb-8">
        {product.collection ? (
          <>
            <button
              onClick={() =>
                product.collection.includes("product")
                  ? router.push("/products")
                  : router.push(`/art/${product.collection}`)
              }
                ...
                {product.materials.map((material) => (
                  ...
                  >
                    <div className="w-16 h-16 bg-gray-400 rounded flex items-center justify-center">
                      <span className="text-xs text-center font-semibold text-gray-700 px-1">
                        {material.name}
                      </span>
                    </div>
                  </button>
              ...
              >
                {product.sizes.map((size) => (
                  <option key={size.id} value={size.id}>
                    {size.name}{" "}
                    {size.price ? `(+$${size.price.toFixed(2)})` : ""}
                  </option>
                ))}
              </select>
            </div>
              ...
              >
                {product.styles.map((style) => (
                  <option key={style.id} value={style.id}>
                    {style.name}{" "}
                    {style.price ? `(+$${style.price.toFixed(2)})` : ""}
                  </option>
                ))}
              </select>
            </div>
